...
class SeHGNN(nn.Module):
    ...
    def __init__(self, dataset, nfeat, hidden, nclass, feat_keys, label_feat_keys, tgt_type,
                 dropout, input_drop, att_drop, n_fp_layers, n_task_layers, act,n_layers,
        ...
        # 使用循环控制 MLP 层数
        layers = []
        layers.append(nn.Linear(hidden, 128))
        layers.append(nn.ReLU())
        for _ in range(n_layers - 1):
            layers.append(nn.Linear(128, 128))
            layers.append(nn.ReLU())
        layers.append(nn.Linear(128, hidden))

        self.semantic_fusion = nn.Sequential(*layers)
    ...
    def forward(self, batch, feature_dict, label_dict={}, mask=None):
        ...
        x = self.feature_projection(x)
         # 使用新的 MLP 模块
        x = self.semantic_fusion(x).transpose(1, 2) # torch.Size([974, 512, 17])
