...
sys.path.append('/home/jpf/csx/SeHGNN-master')
...
def load_dataset(args):
    # dl = data_loader(f'/home/jpf/csx/OpenHGNN-main/openhgnn/dataset/dblp4MAGNN/graph.bin')
    dl = data_loader(args.dataset)

    # use one-hot index vectors for nods with no attributes
    # === feats ===
    features_list = []
    for i in range(len(dl.nodes['count'])):  #'total': 26128, 'count': Counter({1: 14328, 2: 7723, 0: 4057, 3: 20}  'shift': {0: 0, 1: 4057, 2: 18385, 3: 26108}
        th = dl.nodes['attr'][i]
        if th is None:
    ...
    """取出来具体哪些节点被用于trian 、 test"""
    trainval_nid = np.nonzero(dl.labels_train['mask'])[0]    #trainval_nid:array([   0,    3,    7, ..., 4047, 4050, 4051])  trainval_nid.shape:(1217,)
    test_nid = np.nonzero(dl.labels_test['mask'])[0]

    """ 存每个节点的label """
    init_labels[trainval_nid] = dl.labels_train['data'][trainval_nid]  #dl.labels_train['data'].shape:(26128, 4)  init_labels.shape:(4057, 4)  init_labels[trainval_nid].shape:(1217, 4)
    init_labels[test_nid] = dl.labels_test['data'][test_nid]  #init_labels[test_nid].shape：(2840, 4)    dl.labels_test['data'].shape:(26128, 4)
    if args.dataset != 'IMDB':
        init_labels = init_labels.argmax(axis=1)
    init_labels = torch.LongTensor(init_labels)  # init_labels.shape:torch.Size([4057])
    ...
    for i, (k, v) in enumerate(dl.links['data'].items()):
        v = v.tocoo()
        src_type_idx = np.where(idx_shift > v.col[0])[0][0] - 1  #v.row[0]:0  (array([2, 3, 4]),)
        dst_type_idx = np.where(idx_shift > v.row[0])[0][0] - 1  #v.col[0]:6421  (array([2, 3, 4]),)
        row = v.row - idx_shift[dst_type_idx]  # array([   0,    0,    1, ..., 4054, 4055, 4056], dtype=int32)  row.shape:(19645,)
        col = v.col - idx_shift[src_type_idx]  # array([ 2364,  6457,  2365, ..., 13891, 13891, 13892], dtype=int32) col.shape:(19645,)
